  ...
  //        $("#" + visibleContainerId, me.slideBar.contentDocument).before("#" + sheetContainerId, me.slideBar.contentDocument);
    ...
    this.showNote = function (sheetId, pageId, pageUrl, pageTitle, note, getPageHtml, isNewPage){
      ...
      // Via JQuery, remove unnecessary HTML code from the displayed annotation. 
      // Starting point: note_hncontent + full note ID.
      var noteHnContentId = Utils.assembleID(sheetId, pageId, note.noteGuid, EDITOR_);
      $("#" + noteHnContentId + " a", me.slideBar.contentDocument).attr("target", "_blank");
    ...
    function initAuthenticatedUser(){
      ...
      if (me.user && me.user.logged){
        // Get the sheets from storage.
      	...
      	var tmpArray = me.storage.getHooverSheets(me.user);
        // Pass the sheets plus the ID of the active sheet.
        if (tmpArray && tmpArray.length > 0){
          Utils.log(1, "ctrl.initAuthenticatedUser: " + tmpArray.length + " sheets");
//          for (var i = 0; i < tmpArray.length; i++){
          for (var i = tmpArray.length - 1; i > -1; i--){
            me.view.showSheet(tmpArray[i]); // 4a) initializes
          }
        } else {
